...
If your Grafana instance is behind a reverse proxy you should also set the following:

```
    GF_SERVER_ROOT_URL: "https://grafana.company"
```

If you encounter an infinite loop or receive an error indicating that Grafana cannot retrieve the token, please update the two environment variables listed below. Use the local IP address of Authentik instead of the public domain:
